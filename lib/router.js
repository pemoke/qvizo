...
DisplayQuizResultsController = RouteController.extend({
    template: 'quizResult',

    data: function() {
        var quizLocalStorageArray = JSON.parse(localStorage.getItem('QuizzesLocal'));
        quizLocalStorageArray = _.findWhere(quizLocalStorageArray, {'_id': this.params._id});

        var answersLocalStorageArray = JSON.parse(localStorage.getItem('Answers'));
        answersLocalStorageArray = answersLocalStorageArray ?
            answersLocalStorageArray[this.params._id] : null;

        return ({
            quiz: quizLocalStorageArray,
            answers: answersLocalStorageArray
        });
    }
});
...
Router.map(function () {
    ...
    this.route('theQuizResult', {
        path: '/quizResult/:_id',
        controller: DisplayQuizResultsController
    });
});
